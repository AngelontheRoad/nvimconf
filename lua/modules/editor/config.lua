...
function config.hop()
	require("hop").setup({
		...
		jump_on_sole_occurrence = true, -- if only hint a single item, automatically jump to it
		-- case_insensitive = false,  -- respects `smartcase`, no need to set
...
function config.dap()
	...
	-- Map L to hover while session is active
	dap.listeners.after["event_initialized"]["me"] = function()
		for _, buf in pairs(api.nvim_list_bufs()) do
			local keymaps = api.nvim_buf_get_keymap(buf, "n")
			for _, keymap in pairs(keymaps) do
				if keymap.lhs == "L" then
					table.insert(keymap_restore, keymap)
					api.nvim_buf_del_keymap(buf, "n", "L")
				end
			end
		end
